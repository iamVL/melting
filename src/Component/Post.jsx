...
const Post = ({ post, type, loadPosts }) => {
 ...
 // Toggle/Remove existing "dislike"
   ...
   if (showTags) {
     if (post.reactions.length > 0) {
       for (let i = 0; i < post.reactions.length; i++) {
         if (
           post.reactions[i].reactorID === sessionStorage.getItem("user")
         ) {
 ...
 // We'll prepend "// " to the comment text
 const commentDisplay = () => (
   <div className="comment-block">
     <div className="tag-block">
       <button value="tag post" onClick={() => setShowTags((prev) => !prev)}>
         tag post
       </button>
     </div>
 ...
 // Format: user: gj
 // Then on next line: // the comment text
 return (
